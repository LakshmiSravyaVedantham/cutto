...
def generate_video(prompt: str, output_path: str, duration_seconds: int = 5) -> str:
    ...
    logger.info(f"[Veo] Starting video generation (duration={capped_duration}s, model={VEO_MODEL})")
    ...
    logger.info(f"[Veo] Polling finished — done={operation.done}, total_time={total_time:.1f}s")
    ...
    logger.info(f"[Veo] Video saved: {output_path} ({len(video_bytes):,} bytes, {total_time:.1f}s total)")
    return output_path
